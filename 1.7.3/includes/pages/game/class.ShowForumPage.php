...
class ShowForumPage extends AbstractPage 
{
	...
	{
		parent::__construct();
		require('includes/classes/class.Forum.php');
		$this->ticketObj	= new Forum;
	...
	public function show()
	{
		...
		$ticketResult	= $GLOBALS['DATABASE']->query("SELECT t.*, COUNT(a.ticketID) as answer FROM ".FORUM." t INNER JOIN ".FORUM_ANSWER." a USING (ticketID) WHERE t.ownerID = ".$USER['id']." GROUP BY a.ticketID ORDER BY t.ticketID DESC;");
		$ticketList		= array();
		...
		$this->display('page.forum.default.tpl');
	...
	function create() 
	{
		...
		$this->display('page.forum.create.tpl');		
	}
	...
	function send() 
	{
		...
		if(empty($message)) {
			if(empty($ticketID)) {
				$this->redirectTo('game.php?page=forum&mode=create');
			} else {
				$this->redirectTo('game.php?page=forum&mode=view&id='.$ticketID);
			}
		}
		...
		if(empty($ticketID)) {
			...
			$ticketDetail	= $GLOBALS['DATABASE']->getFirstCell("SELECT status FROM ".FORUM." WHERE ticketID = ".$ticketID.";");
	...
	function view() 
	{
		...
		$ticketID			= HTTP::_GP('id', 0);
		$answerResult		= $GLOBALS['DATABASE']->query("SELECT a.*, t.categoryID, t.status FROM ".FORUM_ANSWER." a INNER JOIN ".FORUM." t USING(ticketID) WHERE a.ticketID = ".$ticketID." ORDER BY a.answerID;");
		$answerList			= array();
		...
		$this->display('page.forum.view.tpl');
