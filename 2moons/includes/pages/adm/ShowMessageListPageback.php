 ...
 * @info $Id: ShowMessageListPage.php 2098 2011-12-23 15:50:54Z slaver7 $
 * @link http://code.google.com/p/2moons/
 */

if (!allowedTo(str_replace(array(dirname(__FILE__), '\\', '/', '.php'), '', __FILE__))) exit;

function ShowMessageListPageBack()
{
	global $db, $LNG;
	...
	if ($Selected < 100)
		$Mess      = $db->countquery("SELECT COUNT(*) FROM ".MSGBACK." WHERE `message_type` = '".$Selected."' AND `message_universe` = '".$_SESSION['adminuni']."';;");
	elseif ($Selected == 100)
		$Mess      = $db->countquery("SELECT COUNT(*) FROM ".MSGBACK." WHERE `message_universe` = '".$_SESSION['adminuni']."';;");
	...
	if ($_POST['delsel'] && is_array($_POST['sele']))
	{
		if ($DelSel == true)
		{
			...
			{
				if ($Value = "on")
					$db->query("DELETE FROM ".MSGBACK." WHERE `message_id` = '". $MessId ."';");
			}
		}
	}
	...
	if ($DelDat == true && $_POST['deldat'] && $_POST['sele'] >= 1 && is_numeric($_POST['selday']) && is_numeric($_POST['selmonth']) && is_numeric($_POST['selyear']))
	{
		...
		if ($LimitDate !== false)
		{
			$db->multi_query("DELETE FROM ".MSGBACK." WHERE `message_time` <= '".$LimitDate."';DELETE FROM ".RW." WHERE `time` <= '".$LimitDate ."';");
		}
	}
	...
	if ($Selected == 50)
		$Messages            = $db->query("SELECT * FROM ".MSGBACK." WHERE `message_type` = '". $Selected ."' AND `message_universe` = '".$_SESSION['adminuni']."' ORDER BY `message_time` DESC LIMIT ". $StartRec .",25;");
	elseif ($Selected == 100)
		$Messages            = $db->query("SELECT u.username, m.* FROM ".MSGBACK." as m, ".USERS." as u WHERE m.`message_owner` = u.`id` AND m.`message_universe` = '".$_SESSION['adminuni']."' ORDER BY `message_time` DESC LIMIT ". $StartRec .",25;");
	else
		$Messages            = $db->query("SELECT u.username, m.* FROM ".MSGBACK." as m, ".USERS." as u WHERE m.`message_type` = '". $Selected ."' AND m.`message_owner` = u.`id` AND `message_universe` = '".$_SESSION['adminuni']."' ORDER BY `message_time` DESC LIMIT ". $StartRec .",25;");
